...
func (g *Graph) Filters() []*Context {
	header := reflect.SliceHeader{
		Data: uintptr(unsafe.Pointer(g.filters)),
		Len:  int(g.nb_filters),
		Cap:  int(g.nb_filters),
	}

	return *((*[]*Context)(unsafe.Pointer(&header)))
}

func (g *Graph) SetNbThreads(n int) {
	g.thread_type = C.int((1 << 0))
	g.nb_threads = C.int(n)
}